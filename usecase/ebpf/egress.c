...
BPF_PERF_OUTPUT(egress6_events);
...
BPF_TABLE("extern", struct policy_key, struct policy_val, policy_list, 256);
...
int egress(struct __sk_buff *skb) {
  ...
  struct egress_t event = {};
  struct egress6_t event6 = {};
  ...
  if (eth->h_proto == htons(ETH_P_IP)) {
    ...
    // Identify the container ID from the source IP address.
    struct container_ip_t ip_key = {};
    // docker0 is a host byte order (little endian) because it is located
    ...
    // Identify the container ID from the source IP address.
    struct container_ip_t ip_key = {};
    bpf_probe_read_kernel(&ip_key.ipv6, sizeof(ip_key.ipv6), &iph->saddr);
  ...
  if (proto != IPPROTO_TCP && proto != IPPROTO_UDP) {
    if (eth->h_proto == htons(ETH_P_IP)) {
      ...
      egress_events.perf_submit_skb(skb, skb->len, &event, sizeof(event));
    } else if (eth->h_proto == htons(ETH_P_IPV6)) {
      event6.action = PASS;
      event6.sport = ntohs(event6.sport);
      event6.dport = ntohs(event6.dport);
      egress6_events.perf_submit_skb(skb, skb->len, &event6, sizeof(event6));
    }
    return TC_ACT_OK;
  }
  ...
  // If nothing is specified in the policy except the container name and process
  // name, allow all communication to that process.
  ...
  if (eth->h_proto == htons(ETH_P_IP)) {
    ...
    policy_k.local_port = ntohs(event.sport);
    policy_v = policy_list.lookup(&policy_k);
    if (policy_v) {
      goto pass;
    }
    policy_k.remote_ip = event.daddr;
    policy_v = policy_list.lookup(&policy_k);
    if (policy_v) {
      goto pass;
    }
    ...
    // Check in the order of protocol, remote_port
    policy_k.local_port = 0;
    policy_k.remote_ip = 0;
    policy_v = policy_list.lookup(&policy_k);
    if (policy_v) {
    ...
    // Check remote_ip, remote_port in this order
    policy_k.protocol = 255;
    policy_k.local_port = 0;
    policy_k.remote_ip = event.daddr;
    policy_k.remote_port = 0;
    ...
    policy_k.local_port = ntohs(event6.sport);
    policy_v = policy_list.lookup(&policy_k);
    if (policy_v) {
      goto pass6;
    }
    ...
    policy_k.remote_port = ntohs(event6.dport);
    policy_v = policy_list.lookup(&policy_k);
    if (policy_v) {
      goto pass6;
    }
