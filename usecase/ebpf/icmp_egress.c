...
BPF_PERF_OUTPUT(icmp_egress);
BPF_PERF_OUTPUT(icmp_egress6);
...
BPF_TABLE("extern", struct icmp_policy_key, struct icmp_policy_val,
...
int egress(struct __sk_buff *skb) {
  ...
  struct egress_icmp icmp_event = {};
  struct egress6_icmp icmp_event6 = {};
  ...
  if (isSHOT) {
    if (eth->h_proto == htons(ETH_P_IP)) {
      ...
      icmp_egress.perf_submit_skb(skb, skb->len, &icmp_event,
                                  sizeof(icmp_event));
    } else if (eth->h_proto == htons(ETH_P_IPV6)) {
      icmp_event6.action = DROP;
      icmp_egress6.perf_submit_skb(skb, skb->len, &icmp_event6,
                                   sizeof(icmp_event6));
    }
    return TC_ACT_SHOT;
  }
  ...
  if (eth->h_proto == htons(ETH_P_IP)) {
    ...
    policy_ik.remote_ip = icmp_event.daddr;
    policy_iv = icmp_policy_list.lookup(&policy_ik);
    if (policy_iv) {
      goto pass;
    }
    ...
    icmp_event.saddr = ntohl(icmp_event.saddr);
    icmp_event.daddr = ntohl(icmp_event.daddr);
    icmp_egress.perf_submit_skb(skb, skb->len, &icmp_event, sizeof(icmp_event));
    return TC_ACT_SHOT;
  } else if (eth->h_proto == htons(ETH_P_IPV6)) {
    // Check in the order of version type code remote_IP
